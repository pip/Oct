#!/usr/bin/perl
# 43KKNlxM:Games::Cards::Poker.pm by PipStuart <PipStuart@GMail.Com> to provide useful Poker (&& BlackJack) card-game functions using only Perl;
...
our %EXPORT_TAGS = ('all'  => [ qw(Shuffle Deck SortCards ShortHand
                    ...
                    'blak' => [ qw(Shuffle Deck SortCards ShortHand BJ BJCmp BJHandScore %pd8a) ],
                    'pd8a' => [ qw(                %pd8a   %zdnh %zplf %zloh                  ) ] );
#+-As-------++-2s-------++-3s-------++-4s-------++-5s-------++-6s-------++-7s-------++-8s-------++-9s-------++-Ts-------++-Js-------++-Qs-------++-Ks-------+
#|       s  ||     s    || 3333333  ||      44  ||5555555555||    6666  ||7777777777||   8888   ||  99999   ||TTTTTTTTTT||    JJJJJJ||   QQQQ   ||KK      KK|
#|      s s ||     s    ||33     33 ||     444  ||55        ||  666     ||        77|| 88    88 || 99   99  ||    TT    ||        JJ||  QQ  QQ  ||KK    KK  |
#|     s   s||    s s   ||        33||    4444  ||55        || 66       ||       77 ||88      88||99     99 ||    TT    ||        JJ|| QQ    QQ ||KK  KK    |
#|      sss ||   s   s  ||       33 ||   44 44  ||55 55555  ||66        ||       77 || 88    88 ||99      99||    TT    ||        JJ||QQ      QQ||KKKK      |
#|  AA   s  ||  s     s ||  333333  ||  44  44  ||5555   55 ||66  66666 ||      77  ||  888888  || 999  9999||    TT    ||        JJ||QQ      QQ||KKK       |
#| AAAA sss || s       s||       33 || 44   44  ||        55||66666   66||      77  || 88    88 ||   999  99||    TT    ||        JJ||QQ  QQ  QQ||KKKK      |
#|AA  AA    || s  s s  s||        33||4444444444||        55||66      66||     77   ||88      88||       99 ||    TT    ||        JJ||QQ   QQ QQ||KK  KK    |
#|AAAAAA    ||  ss s ss ||        33||      44  ||        55||66     66 ||     77   ||88      88||       99 ||    TT    ||        JJ|| QQ   QQQ ||KK   KK   |
#|AA  AA    ||     s    ||33     33 ||      44  ||555    55 || 66   66  ||    77    || 88    88 ||      99  ||    TT    ||       JJ ||  QQ  QQQ ||KK     KK |
#|AA  AA    ||    sss   || 3333333  ||      44  ||  555555  ||  66666   ||    77    ||  888888  ||   9999   ||    TT    ||  JJJJJJ  ||   QQQQ QQ||KK      KK|
#+----------++----------++----------++----------++----------++----------++----------++----------++----------++----------++----------++----------++----------+
...
#|     c c c||    ccc   ||a whole   ||          ||          ||          ||          ||          ||          ||          ||          ||          ||          |
#|     ccccc||    ccc   ||deck of   ||          ||          ||          ||          ||          ||          ||          ||          ||          ||          |
#|     c c c||  cc c cc ||playing   ||          ||          ||          ||          ||          ||          ||          ||          ||          ||          |
#|   44  c  || ccccccccc||cards fits||  4 44    ||555555    ||  666     ||777777    || 8888     || 9999     ||TTTTTT    ||  JJJJ    || QQQQ     ||KK  KK    |
#|  444 ccc ||  cc c cc ||neatly in || 44 44    ||55        || 66       ||    77    ||88  88    ||99  99    ||  TT      ||    JJ    ||QQ  QQ    ||KK KK     |
#| 4 44     ||     c    ||160x48    ||44  44    ||55555     ||66        ||   77     || 8888     || 99999    ||  TT      ||    JJ    ||QQ  QQ    ||KKK       |
#|444444    ||     c    ||either 6x6||444444    ||    55    ||66666     ||  77      ||88  88    ||    99    ||  TT      ||    JJ    ||QQQ QQ    ||KKKK      |
#|   44     ||    ccc   ||or 10x10  ||    44    ||55  55    ||66  66    || 77       ||88  88    ||   99     ||  TT      ||    JJ    ||QQ QQ     ||KK KK     |
#|   44     ||   ccccc  ||each card;||    44    || 5555     || 6666     || 77       || 8888     || 999      ||  TT      || JJJJ     || QQQ Q    ||KK  KK    |
#+----------++----------++----------++----------++----------++----------++----------++----------++----------++----------++----------++----------++----------+
our $VERSION = '1.6';my $d8VS='Q73M4ref'; # remember that 1.10 is really the float 1.1 and not automatically 1.010 or something
...
sub BJHandScore { # returns the score of the passed in @hand or ShortHand for BlackJack
  my @hand = @_; return(-1) unless(@hand); my $shrt; my $scor; my $aenc=0;
  my $aflg = 0; $aflg = 1 if(ref($hand[0]) eq 'ARRAY'); my $aref;
  if($aflg        ) { $aref =    $hand[0];          }
  else              { $aref =   \@hand;             }
# SortCards($aref) unless(@{$aref} == 1); # make sure cards are sorted first?
  if(@{$aref} == 1) { $shrt =  $aref->[0];          }
  else              { $shrt =    ShortHand($aref);  } # may auto-call SortCards 1st?
  $shrt =~ s/s$//; # BlackJack doesn't care about suitedness flags so strip any off
  for(split(//, $shrt)) {
    if   (/^\d$/    ) { $scor += $_; }
    elsif(/^[TJQK]$/) { $scor += 10; }
    elsif(/^A$/     ) { $scor++;$aenc++; } } $scor += 10 if($scor <= 11 && $aenc);
  return($scor); }
sub BJCmp { # Similar to num <=> or str cmp between the two BlackJack scores passed as parameters
  my $scr0 = shift(@_) || 0; my $scr1 = shift(@_) || 0;
...
%pd8a = ( # Global Poker Data    # possible ShortHand holes, flops, and hands (which used to include much redundant ways data that is now generated as needed)
  ...
  'namz' => { # hand score range starts and names
       '0' =>     'Royal_Flush',   #  Royal     Flush    0           1
       '1' =>  'Straight_Flush',   #  Straight  Flush    1..   9     9
      '10' =>  'Four-of-a-Kind',   #  Four -of-a-Kind   10.. 165   156
     '166' =>  'Full_House'    ,   #  Full      House  166.. 321   156
     '322' =>           'Flush',   #            Flush  322..1598  1277
    '1599' =>  'Straight'      ,   #  Straight        1599..1608    10
    '1609' => 'Three-of-a-Kind',   #  Three-of-a-Kind 1609..2466   858
    '2467' =>   'Two_Pair'     ,   #  Two        Pair 2467..3324   858
    '3325' =>   'One_Pair'     ,   #  One        Pair 3325..6184  2860
    '6185' =>  'High_Card'     ,}, #  High       Card 6185..7461  1277
  'bjds' => [ qw( 2 3 4 5 6 7 8 9 T A ) ], # BlackJackDealerShowing (10 from 2..11 [Ace]);
  'bjph' => [ qw( 22 33 44 55 66 77 88 99 TT AA   l9 9 10 11 12 13 14 15 16 gG   A2 A3 A4 A5 A6 A7 A8 A9 ) ], # BlackJackPlayer'sHands(28) ;
  'bjat' => [ # BlackJackActionTable with first dimension DealerShowing 2..A, 2nd dimension Player'sHand (Actionz: sPlit, Doubledown, Hit, Stand, suRrender) ;
              [ qw( P P H D H P P P S P  H H D D H S S S S S  H H H H H D S S ) ],    # DS 2  ## Note:Table cp of HTTPS://WikiHow.Com/Sample/Blackjack-Chart ;
              [ qw( P P H D H P P P S P  H D D D H S S S S S  H H H H D D S S ) ],    # DS 3  ## Just inverted to flip rows with columns for DS 1st dimeNsion;
              [ qw( P P H D H P P P S P  H D D D S S S S S S  H H D D D D S S ) ],    # DS 4  ## Hopefully these are close to optimal play strategiez for BJ!;
              [ qw( P P P D H P P P S P  H D D D S S S S S S  D D D D D D S S ) ],    # DS 5  ## Blox of Action groupz go in Ten,then another Ten, then Eight;
              [ qw( H H H H H H P S S P  H H H D H H H H R S  H H H H H H S S ) ], ], # DS A
# The importance of blackjack table selection: 1st, U nEd2select the bSt online blackjack tAbl2improve your chances. These are some key points 2 bear in mind:
...
# To summarize, before you play, check how many decks are used, the payout on blackjack, and the rules on doubling down, splitting cards, and surrendering.
#   It’s important to understand how these different elements help or hurt you.  ## Note:This above block of edgez also comes from the PlayUSA link above;
# According to: HTTPS://BlackJackInSight.Com/when-to/surrender/ "You should surrender in blackjack when you’re holding a hard 16 (but not a pair of 8s)
#   against a dealer’s 9, 10, or Ace, or when you have a hard 15 versus a dealer’s 10. By folding in these specific spots, you cut your losses and
#   lower the house edge, keeping more of your bankroll intact over the long run."   ## Note:I think I may upd8 the ActionTable to include these R conditionz;
# According to: HTTPS://WizardOfOdds.Com/games/blackjack/surrender/ "Blackjack Surrender: What it is and When to do it; Introduction
#   What is surrender in blackjack? This is a strategy in which the player may fold the hand risking only half of the bet rather than the entire amount.
#   There are two options in terms of surrendering your hand. Late Surrender may be done after the dealer checks for blackjack and Early Surrender is
#   possible before the dealer check. This page takes an in-depth look at the choice to surrender in blackjack. It depends on lots of things, whether
#   the dealer takes a hole card, the number of decks, whether the dealer hits or stands on soft 17, and sometimes the composition of the player's hand.
#   The purpose of this page is to give exact advice to the basic-strategy player on when to surrender. When the tales show a total of 16, this refers
#   to either a 10+6 or 9+7 total. The situation of two eights has a separate row when surrender is appropriate.
#   Late surrender means that the player may surrender his hand and lose half his bet only after the dealer checks for blackjack. The following basic
#   strategy tables show when to surrender (Y=yes, N=no) depending on the player's total, dealer's up card, and the number of decks. The right two
#   columns represent a dealer ace according to whether the dealer stands (S17) or hits (H17) a soft 17.  ## Note:followed by tablez 4 1,2,4+ deckz/shoe;
#   Early surrender is a seldom found rule in which the player may forfeit his hand and half his bet before the dealer checks for blackjack. The strategy
#   is different from late surrender only when the dealer has a ten or ace showing. Following is a list for when to take early surrender against a dealer
#   10 or ace, regardless of the rules, except as noted.
...
# According to: HTTPS://ToolsGambling.Com/blog/blackjack-surrender-explained (which has gr8 articlez && calQl8orz wi many upd8d 2026) suRrender a hard 16
#   (i.e., not 8-8, which you should split against Dealer Ten showing && mAB alsO AgNst Dlr 9 or Ace 2). Also suRrender hard 15 against just Dealer Ten;
...
Games::Cards::Poker - Pure Perl Poker (&& BlackJack) functions

=head1 VERSION

This documentation refers to version 1.6 of Games::Cards::Poker, which was released on Wed Jul  1 09:37:45:36 -0500 2026.
...
cards and manipulating them for simple Poker games or simul8ions.
...
* removed indented POD links from new SUPPORT section since they're showing literal

* made dist and uploaded new release, then closed the RT WorstHand bug
...
* audited all of old MANIFEST, yanked SuitAbstraction.pm, fixed t/*.t, && it seems like none of following old bin files are really needed until maybe l8r
  ...
  bin/tstPoker.pl  - generate test runs of Texas Hold'Em poker hands and print out the results (probably precursor to ex/txholdem.pl)
...
You can also look for information at:
  ...
  HTTPS://RT.CPAN.Org/NoAuth/Bugs.html?Dist=Games-Cards-Poker

    AnnoCPAN: Annotated CPAN documentation

  HTTP://AnnoCPAN.Org/dist/Games-Cards-Poker

    CPAN Ratings

  HTTPS://CPANRatings.Perl.Org/d/Games-Cards-Poker

    Search  CPAN

  HTTP://Search.CPAN.Org/dist/Games-Cards-Poker

=head1 LICENSE

Most source code should be Free! Code I have lawful authority over is and shall be!
Copyright: (c) 2004-2026, Pip Stuart.
