...
use strict;use warnings;use utf8;use v5.22;use vars qw( $AUTOLOAD );use open qw(:std :encoding(UTF-8));#se POSIX qw(locale_h);use locale;setlocale(LC_ALL,'');
use        Curses;use Tie::Array; # HTTPS://StackOverFlow.Com/questions/71880471/ncurses-65536-color-pairs says ncurses can be compiled w/ --enable-ext-colors
my $curs=8; # used to set this to 0 for old 4NT rendering so now should remove all tests for it                                       # && --enable-widec ;
...
# no strict 'subs'; # could also inside block braces { no warnings 'once'; for each alias }
  ...
  *DefaultValues         = \&DfltValu;#*DefaultValues        if 0; # maybe these should all go in BEGIN… I'm trying that
  *MakeMethods           = \&MkMethdz;#*MakeMethods          if 0;
  *InitializeColorPair   = \&InitPair;#*InitializeColorPair  if 0;
  *PrintBorderCharacter  = \&BordChar;#*PrintBorderCharacter if 0;
  *ConvertAnsiColorCode  = \&CnvAnsCC;#*ConvertAnsiColorCode if 0;
  *ShockScreen           = \&ShokScrn;#*ShockScreen          if 0;
  *KeyNumbers            = \&KNum    ;#*KeyNumbers           if 0;
  *ColorLetters          = \&CLet    ;#*ColorLetters         if 0;
  *NumColors             = \&NumC    ;#*NumColors            if 0;
  *Height                = \&Hite    ;#*Height               if 0;
  *Width                 = \&Widt    ;#*Width                if 0;
  *PrintString           = \&Prnt    ;#*PrintString          if 0;
  *DrawWindow            = \&Draw    ;#*DrawWindow           if 0;
  *WaitTime              = \&Wait    ;#*WaitTime             if 0;
  *GetKey                = \&GetK    ;#*GetKey               if 0;
  *GetString             = \&GetS    ;#*GetString            if 0;
  *MoveCursor            = \&Move    ;#*MoveCursor           if 0;
  *ResizeWindow          = \&Rsiz    ;#*ResizeWindow         if 0;
  *UpdateWindow          = \&Updt    ;#*UpdateWindow         if 0;
  *MessageWindow         = \&Mesg    ;#*MessageWindow        if 0;
  *PromptWindow          = \&Prmt    ;#*PromptWindow         if 0;
  *FocusWindow           = \&Focu    ;#*FocusWindow          if 0;
  *ColorPickWindow       = \&CPik    ;#*ColorPickWindow      if 0;
  *BrowseWindow          = \&Brws    ;#*BrowseWindow         if 0;
  *DeleteWindow          = \&DelW    ;#*DeleteWindow         if 0;
  *DelW                  = \&DESTROY ;#*DelW                 if 0; # probably unneeded for DelW used twice
}
...
#  $self->Move($self->YCrs()+1,0) unless(($_+1) % 32);}                   $self->Move(0,0);$self->{'_wind'}->refresh(); $self->GetK(-1);
  if(has_colors()){unless(exists($GLBL{'COLRPNDX'})){$GLBL{'COLRPNDX'}=1;$GLBL{'COLRPARS'} = {'^-1'=>0};$GLBL{'COLRPARY'} = ['^-1'];} # init just A_NORMAL
    if($fgcl eq '-1'){$curp=$self->{'_valullsp'} if(defined($self->{'_valullsp'}));$bold=$self->{'_valullsb'} if(defined($self->{'_valullsb'}));} #go bak2last
      ...
      if($fgcl =~ /^([FDAL]):?([0-9A-Z._])$/i && exists $p8k2{uc($1)} && exists $p8k2{uc($1)}{$2}){$fcan=$p8k2{uc($1)}{$2};}
      if($bgcl =~ /^([FDAL]):?([0-9A-Z._])$/i && exists $p8k2{uc($1)} && exists $p8k2{uc($1)}{$2}){$bcan=$p8k2{uc($1)}{$2};} # can't take b: for bkgr,need F;
...
#       $self->{'_wind'}->attr_off( COLOR_PAIR($self->{'_valulasp'}),undef) if(defined($self->{'_valulasp'}));
#       $self->{'_wind'}->attr_on(  COLOR_PAIR($curp),undef);
        $self->{'_wind'}->attr_set(A_NORMAL,   $curp ,undef);
      }
      if( $bold){$self->{'_wind'}->attr_on(A_BOLD  ,undef);}else{$self->{'_wind'}->attr_off(A_BOLD  ,undef);} # not sure if this is right way to get Dflt grn,
      if(!$curp){$self->{'_wind'}->attr_on(A_NORMAL,undef);}else{$self->{'_wind'}->attr_off(A_NORMAL,undef);} #   but might need2attroff NORM earlier, if curp
  ...
  unless($GLBL{'FLAGOPEN'}){ $GLBL{'FLAGOPEN'} = 1;
   #use_env(0); # raw() allows ^C,^S,^Z 2simply pass thru,unlike cbreak(),but raw requirz`reset`from the cmdline,if the app crashes; napms($ms) 2nap millisecs;
    ...
    @kndx = ( # J54MLRes:Oni's Curses::Simp died BUTTON[1..4]_RESERVED_EVENT ;
      ...
      KEY_EOS                , KEY_EXIT, # KEY_EVENT  , KEY_EXIT              ,
...
sub CScr{ # Close previously OpenedCursesScreen # Following are Curses funcs that might be useful to call in CloseScreen(): termname(),erasechar(),killchar()
  if($curs && $GLBL{'FLAGOPEN'}){${$DISPSTAK[-1]}->DelW() while(@DISPSTAK);$GLBL{'FLAGOPEN'}=0; # delete all simp objects before end
    delwin($_[0]->{'_wind'}) if (defined($_[0]) && exists($_[0]->{'_wind'})); # && ref($_[0]) eq 'Curses::Simp');
    endwin();
  } return(); }
...
sub Prnt{ # Simp object PrintString method
  ...
  if(exists($parm{'fclr'}) || exists($parm{'bclr'})){ if($parm{'prin'}){ if($self->{'_btyp'}){ if($parm{'ycrs'}){ $parm{'ycrs'}--; }else{ $parm{'zery'} = 1; }
    ...
    while(length($parm{'text'})){ $chrz = substr($parm{'text'}, 0, 1, ''); $delt = 0;
      ...
      if(exists($parm{'ycrs'}) && exists($parm{'xcrs'})){ if($curs){
...
#         $self->{'_wind'}->addstr($chrz);
#         $self->{'_wind'}->printw($chrz);
        }
      }  $cnum++; }
...
#       $self->{'_wind'}->addstr($parm{'text'});
#       $self->{'_wind'}->printw($parm{'text'});
      ...
      for($cnum = 0; $cnum < length($ltxt); $cnum++){
        if($cnum <= $self->{'_widt'}){
          ...
          while($cnum < (length($ltxt) - 1) &&
            ($ordc > 31 && $ordc != 127 && $ordc != 155 && $ordc != 156 && $ordc < 128) &&
            ($ordd > 31 && $ordd != 127 && $ordd != 155 && $ordd != 156 && $ordd < 128) &&
...
#                               printw("%s",                 substr($ltxt, $cnum - $delt, $delt+1));
#             $self->{'_wind'}->addstring($lnum,$cnum-$delt, substr($ltxt, $cnum - $delt, $delt+1) . ''); # not sure why concat with empty str works!
              $self->{'_wind'}->addstring(                   substr($ltxt, $cnum - $delt, $delt+1) . ''); # not sure why concat with empty str works!
#             $self->{'_wind'}->addstring(                          $ltxt                         );
              for my $cndx ($cnum-$delt..$delt+1) {
#               $self->{'_wind'}->addch(  ord(substr($ltxt,$cndx,1)));
#                                 printw("%c",substr($ltxt,$cndx,1) );
              }
            }}}}
      if($curs && $cnum < ($self->{'_widt'} - 2)){my $wmc2=($self->{'_widt'} - $cnum) - 2;$wmc2=0 if($wmc2 < 0);
        $self->{  '_wind'}->addstr(' ' x $wmc2);#$ltxt.=' ' x $wmc2 if(defined($ltxt)); # this WidthMinusCnum&&2 is non-negative to stifle new 5.22 warning
#       while(                                 !$self->{'_btyp'} && #efined($ltxt) &&  #   about negative repeat count for x doing nothing
#         defined($ltxt) &&              length($ltxt) <  $self->{'_widt'}){$ltxt.=' ';
#                                     $self->{'_xcrs'} < ($self->{'_widt'} - 1)){#$ltxt.=' ';
#         $self->{'_wind'}->addstr(' ' ); }
...
#                         printw("%s",$ltxt);refresh();
        $self->{'_wind'}->addstring(  $ltxt);refresh();
#       for my $cndx (0..length($ltxt)-1) {
#         $self->{'_wind'}->addch(  ord(substr($ltxt,$cndx,1)));
#                           printw("%c",substr($ltxt,$cndx,1) );refresh();
#       }
      } else   {print SCRP "\@screen $lnum 0 $ltxt\n";}
      if(1 && length($ltxt) < $self->{'_widt'}){
        if($curs){
          if(length($ltxt) < ($self->{'_widt'} - 2)){$ltxt.=' ' x (($self->{'_widt'} - 2) - length($ltxt));
            $self->{'_wind'}->addstr(                       ' ' x (($self->{'_widt'} - 2) - length($ltxt)));}
#         while(                               !$self->{'_btyp'} &&
#           defined($ltxt) && length($ltxt) <  $self->{'_widt'}     ){$ltxt.=' ';
#                           $self->{'xcrs'} < ($self->{'_widt'} - 1)){#$ltxt.=' ';
#           $self->{'_wind'}->addstr(' '); }
          $self->{'_wind'}->addstr('  ') unless($self->{'_btyp'});
        }
      }
  ...
  if(($lnum < ($self->{'_hite'} - 2)) || # pad blank lines if height not full
    ...
    while($lnum < $self->{'_hite'}){
      ...
      if($curs){$self->{'_wind'}->addstring($ltxt);}
      if($self->{'_btyp'}){
        $self->BordChar('vl');
...
#              $_ = splice(@{$_[0]->[0]->{'_fclr'}}, @_[1..$#_]); $_[0]->[0]->Curses::Simp::TestDraw() if($_[0]->[0]->{'_flagadtf'}); return($_); }
  ...
  $self->[0]->Curses::Simp::TestDraw() if($self->[0]->{'_flagadtf'});
  return($retn);
}
sub EXTEND   { }
