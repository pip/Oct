...
our $VERSION    ='1.82';our $d8VS='O31LGxdA'; # remember that 1.10 is really the float 1.1 and not automatically 1.010 or something, so really must be > 1.8
...
sub DoW{ # calculate the Day-of-the-Week from the Year, Month, and Day or now if no parameters given and no Month or Day become 1
  ...
  $daay++ if(($year == 2024 &&  $mnth >=  3) || ($year == 2025 && $mnth <= 2)); # silly kludge hack to shift right between Feb.29..28leap-dayz for 2024 AgN!;
  return(($daay + (31 * int((12 * $mndx) + $mnth - 2)) / 12) % 7);} # HTTPS://WikiHow.Com/Calculate-the-Day-of-the-Week mA hav betr wAz 2 du this? ;
...
This documentation refers to version 1.82 of Time::DayOfWeek, which was released on
.
