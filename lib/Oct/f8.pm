# E7TMLKEY: Oct::f8.pm by PipStuart <Pip@CPAN.Org> to provide general f0nt && pal8 objects && files for usage, management, && manipul8ion of present8ion;
package     Oct::f8;
...
use         Oct::a8;
use         Oct::b8;
#se         Oct::c8; # c8 needs to use f8 more than vice versa for now, but eventually I'll want interactive f8 object editing, maybe as a subclass
use         Oct::d8;
use         Carp;
#ur        @EXPORT= qw();
...
#for(keys(%libz)){if(-d "$_/Oct/f8/f0nt"){push(@lfls,<"$_/Oct/f8/f0nt/*.f0nt">);}
#                 if(-d "$_/Oct/f8/pal8"){push(@lpls,<"$_/Oct/f8/pal8/*.pal8">);}}
...
sub new{my($nvkr,$ityp,$idat)=@_;my $nobj=ref($nvkr);
  ...
  my $self=bless({},$clas); # maybe should also allow construction from filenames with assumed extension or can be found in default ~/lib/Oct/f8/
...
#   crE8 what's needed for new `f0nt` util (like `pal8`) to enable existing f0nt files to maybe become executable also so I can add ~/lib/Oct/f8/f0nt/
