# H92MLLRe: Oct::p8::nvg8.pm crE8d by PipStuart <Pip@CPAN.Org> to be a class for my general path-finding implement8ions of A*, Dijkstra's, GBFS, etc.;
...
use         Oct::a8;
use         Oct::p8::m8rx;
use         Oct::p8::tabh; # Tied-Array Binary Heap (as opposed to any other priority queue)
use         Oct::p8::psps::cnst;
use         Curses::Simp;
...
sub lodm{my $self= shift(@_);my $shrn= shift(@_);$self->{'m8xo'}= Oct::p8::m8rx->new();return($self->{'m8xo'}->lodt($shrn));} # very basic LOaD M8rix
sub astr{my $self= shift(@_);my $frxy= shift(@_);my $toxy= shift(@_);               my($fr_x,$fr_y,$to_x,$to_y); # take either cmprSd b64 xy rpos or x,y, x,y
  ...
  if   (defined($fr_x) && defined($fr_y) && defined($to_x)&& defined($to_y)){ # might want to also verify that from && to are not within walls or unwalkable?
    my @tabh;my $tbho=tie(@tabh,'Oct::p8::tabh');my $rang= rang($fr_x,$fr_y,$to_x,$to_y); # altho pathing from Spawn 2 Cntrlr both unwalkable&&need rang!
...
# Oct::p8::nvg8 object constructor as class method or copy as object method. First param can be ref to copy. Not including optional ref from copy,
...
Oct::p8::nvg8 - general path-finding NaViG8ion objects

=head1 VERSION

This document8ion refers to version 0.0 of Oct::p8::nvg8, which was released on
.

=head1 SYNOPSIS

  use utf8;use v5.10;use Oct::p8::nvg8; # maybe just give nvg8 new a gmap roomName to internally load with gmap into m8rx to then call astr member on
  my @tabh; # tied array && basic object
  my $tbho  = tie(@tabh,'Oct::p8::tabh');
  my @m8rx; # tied array && basic object
  my $tm8o  = tie(@m8rx,'Oct::p8::m8rx');
...
Oct::p8::nvg8's constructor to hold navig8ion st8 d8a

=head1 NOTES

I hope you find Oct::p8::nvg8 useful.

=head1 CHANGES

Revision history for Perl extension Oct::p8::nvg8:
...
Please run:
  ...
  `perl -MCPAN -e "install Oct::p8::nvg8"`
...
Oct::p8::nvg8 requires:

=over 2

=item L<Oct::p8::tabh>  - for Tied Array Binary Heap priority queue objects

=item L<Oct::p8::m8rx>  - for tied array M8RiX objects to describe map path costs
...
L<Oct::p8::taqs for TiedArrayQuickSorted or l8r similar classes?>
