# H83M3sV4: Oct::p8::psps::gmap.pm crE8d by PipStuart <Pip@CPAN.Org> as "Pip's Screeps Global Map" to make an object like original JavaScript Game.map;
...
package     Oct::p8::psps::gmap;
use strict; use warnings;use utf8;use v5.10;use Path::Tiny;
use         Oct::a8;
use         Oct::b8;
#se         Oct::c8; # eventually use c8 && Simp && TkAC for similarly rendering anim8ing interactive interfaces for running the local Game simul8ion
#se         Oct::d8;
use         Oct::p8::psps::cnst; # import all the Global Constants
use         Carp; # below mkdir $HOME/lib/Oct/p8/psps/gmap/(\.tmp|simu|prv8|[sp][01][we][sn])/ for 19 dirs:3 solo,Screeps.com orPtr,shrd0or1,&&WS ES WN EN
my  $VERSION='0.0';my $d8VS='HBCL6UTF';            my $mapd="$ENV{'HOME'}/lib/Oct/p8/psps/gmap";
mkdir  ("$ENV{'HOME'}/lib"                 ,0755) unless(-d "$ENV{'HOME'}/lib"                 ); # mk sure proper lib gmap dirz exist
mkdir  ("$ENV{'HOME'}/lib/Oct"        ,0755) unless(-d "$ENV{'HOME'}/lib/Oct"        ); # betr 2 l8r use auto-mkdir module
mkdir  ("$ENV{'HOME'}/lib/Oct/p8"     ,0755) unless(-d "$ENV{'HOME'}/lib/Oct/p8"     );
mkdir  ("$ENV{'HOME'}/lib/Oct/p8/psps",0755) unless(-d "$ENV{'HOME'}/lib/Oct/p8/psps"); # probably all PTR variantz are same
...
# Oct::p8::psps::gmap object constructor as class method or copy as object method. This is planned to be very similar to Screeps global Game.map object.
...
Oct::p8::psps::gmap - Pip's Screeps Global Map class

=head1 VERSION

This document8ion refers to version 0.0 of Oct::p8::psps::gmap, which was released on
.

=head1 SYNOPSIS

  use       Oct::p8::psps::gmap;use v5.10;
  my  $gm = Oct::p8::psps::gmap->new();
...
Oct::p8::psps::gmap's constructor...

=head1 NOTES

I hope you find Oct::p8::psps::gmap useful.

=head1 CHANGES

Revision history for Perl extension Oct::p8::psps::gmap:
...
Please run:
  ...
  `perl -MCPAN -e "install Oct::p8::psps"`
...
Oct::p8::psps::gmap requires:

L<Carp>                      to allow errors to croak() from calling sub

L<Oct::a8>              for autom8d exports of global d8a && functions

L<Oct::b8>              to handle simple number-base conversion

L<Oct::d8>              for compact time stamps

L<Oct::p8::psps::cnst>  for global constants

=head1 SEE ALSO

L<Oct::p8::psps::l8r_subclasses>
