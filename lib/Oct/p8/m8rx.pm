# H8OM842X: Oct::p8::m8rx.pm crE8d by PipStuart <Pip@CPAN.Org> to be a class to store path cost M8RiX objects for nvg8:astr along with other grid map d8a;
...
use         Oct::a8; # needed for single b10 && b64 conversion without full b8 functions (altho a8 Xports so much,just using b8 might actually be faster?)
use         Oct::p8::psps::cnst   ; # needed for just init param dimensions from my fake added ROOM_* constants
use         Oct::p8::psps::gmap   ; # needed for wrapped lodt to popul8 m8rx layer 1 with particular room terrain d8a
...
sub lodt{my $self= shift(@_);my $shrn= shift(@_); # LOaD m8rx layer 1 from gmap Terrain d8a via single compound param of SHard digit code && expanded RoomName
  ...
  my $gmap= Oct::p8::psps::gmap->new();my $td8a=$gmap->lodt($shrn); # should default load 8W8N3 expanded terrain
...
# Oct::p8::m8rx object constructor as class method or copy as object method. First param can be ref to copy. Not including optional ref from copy,
...
Oct::p8::m8rx - path cost M8RiX d8a objects

=head1 VERSION

This document8ion refers to version 0.0 of Oct::p8::m8rx, which was released on
.
...
Oct::p8::m8rx's constructor without tying an array...

=head1 NOTES

I hope you find Oct::p8::m8rx useful.

=head1 CHANGES

Revision history for Perl extension Oct::p8::m8rx:
...
Please run:
  ...
  `perl -MCPAN -e "install Oct::p8::m8rx"`
...
Oct::p8::m8rx requires:

=over 2

=item L<Tie::Array>    - allow text arrays to be bound to objects

=item L<Oct::a8>  - for autom8d exports of global d8a && functions
