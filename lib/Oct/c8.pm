...
sub new{my( $nvkr,$ityp,   $idat)=@_;  my $nobj=ref($nvkr); # basically same as TIEARRAY below just without commented old Curses code (adding fldz stuff)
  ...
  while(@_){my $foun=0;my($keey,$valu)=(shift,shift);
    ...
    unless ($foun){
      ...
      unless($foun){die "!*EROR*! Oct::c8::new initializ8ion key:$keey was not recognized!\n";}
...
# $self->Updt(1); # Updt with NoChange flag would prepare for flagz maxi, shrk, && cntr to call Rsiz to resize window on dynamic dimensions
  if($curs){ #    $self->{'_wind'}=newwin(                                  $self->{'_hite'},      $self->{'_widt'},
    ...
    unless(exists($self->{'_wind'}) && defined($self->{'_wind'})){
      die   "!*EROR*! Oct::c8::new could not crE8 new window with hite:$self->{'_hite'}, widt:$self->{'_widt'}, " .
                                                                      "yoff:$self->{'_yoff'}, xoff:$self->{'_xoff'}!\n";exit();}}
# $self->{'_dndx'}=@DISPSTAK;push(@DISPSTAK,\$self); # add new Oct::c8 object to display order stack
  ...
  unless($self->{'_flagopen'}){ # need Open main Screen for what was a new Curses::Simp obj but should now start initializing SDL here instead
    if($sdlf){
      for my  $fkey(keys(%f8fm)){ # load my top64 f0nts (attempting to loop with $_ messes up inside tied constructor here)
        $GLBL{  '_f8om'}{$fkey}=Oct::f8->new("$ENV{'HOME'}/lib/Oct/f8/f0nt/$f8fm{$fkey}.f0nt") if(!exists($GLBL{'_f8om'}{$fkey}));}
...
sub GetK{my $self=shift;my $tmot=shift||0;my $tkey='-1';my $stix;my $ctix;
  ...
  if(defined($tkey) && length($tkey)){$self->{'_text'}[0]="tkey:$tkey; kmod:" . sprintf("%5d",$self->{'_evnt'}->key_mod) . ';';$self->Draw();} # try 2 put gotten tkey at 0,0 && show it (&& w8?)
  return($tkey);}
...
#       $self->{'_xcrs'}=0 if($self->{'_xcrs'} < 0);} # actually stripping them out of the middle of string && pre-moving cursor will not yield proper results
      ...
      # HTTPS://DGL.CX/2023/09/ansi-terminal-security goes over 20 Years of Escaping vulnerabilities in very many terminal emul8or programs && shud B stuDd;
      # HTTPS://Invisible-Island.Net/xterm/ctlseqs/ctlseqs.html doQmNtz a hUge number of terminal sequencez && their historiez (wich cud hLp upd8 lstz BlO);
      # HTTPS://StackOverFlow.Com/questions/71561226/what-are-7-bit-8-bit-environments-for-control-functions-according-to-iso-iec-6 had answer post with:
      #  CSI (0x9B)(8-bit) becomez   sequence:   0x1B 0x5B     (7-bit)
      ...
      # CSI n?    I :     -   (mAB rushd Xtra <CarriageRetn>?)  (I'm not sure what this is or does yet, but vim sometimes in shell cmndz emitz ^[[I why?)
      ...
      # [pip@OniN4AM9Qw2~/dvl/t8/raku]en '\e]4;1;?\a'                     # [pip@OniN4AM9W9c~/dvl/t8/raku]en '\e]4;1;?\a'
      # ^[]4;1;rgb:5959/1313/1313^G[pip@OniN4AM9R1b~/dvl/t8/raku]         # ^[]4;1;rgb:5959/1313/1313^G[pip@OniN4AM9WD3~/dvl/t8/raku]
      # [pip@OniN4AM9R35~/dvl/t8/raku]en '\e]4;2;?\a'                     # [pip@OniN4AM9WDu~/dvl/t8/raku]en '\e]4;2;?\a'
      # ^[]4;2;rgb:4040/6969/1919^G[pip@OniN4AM9R60~/dvl/t8/raku]         # ^[]4;2;rgb:4040/6969/1919^G[pip@OniN4AM9WI1~/dvl/t8/raku]
      # [pip@OniN4AM9R72~/dvl/t8/raku]en '\e]4;3;?\a'                     # [pip@OniN4AM9WIj~/dvl/t8/raku]en '\e]4;3;?\a'
      # ^[]4;3;rgb:e2e2/9797/3f3f^G[pip@OniN4AM9RAA~/dvl/t8/raku]         # ^[]4;3;rgb:e2e2/9797/3f3f^G[pip@OniN4AM9WLu~/dvl/t8/raku]
      # [pip@OniN4AM9RBF~/dvl/t8/raku]en '\e]4;4;?\a'                     # [pip@OniN4AM9WMe~/dvl/t8/raku]src
      # ^[]4;4;rgb:0e0e/1212/1616^G[pip@OniN4AM9RE0~/dvl/t8/raku]         # [pip@OniN4AM9WQq~/dvl/t8/raku]en '\e]4;0;?\a'
      # [pip@OniN4AM9REs~/dvl/t8/raku]en '\e]4;5;?\a'                     # ^[]4;0;rgb:2020/1414/0808^G[pip@OniN4AM9Wbp~/dvl/t8/raku]
      # ^[]4;5;rgb:5c5c/1c1c/6767^G[pip@OniN4AM9RJ5~/dvl/t8/raku]         # [pip@OniN4AM9WeS~/dvl/t8/raku]en '\e]4;1;?\a'
      # [pip@OniN4AM9RK6~/dvl/t8/raku]en '\e]4;6;?\a'                     # ^[]4;1;rgb:8080/2020/0c0c^G[pip@OniN4AM9Whc~/dvl/t8/raku]
      # ^[]4;6;rgb:0606/9898/9a9a^G[pip@OniN4AM9RNP~/dvl/t8/raku]         # [pip@OniN4AM9Wo6~/dvl/t8/raku]en '\e]4;2;?\a'
      # [pip@OniN4AM9ROK~/dvl/t8/raku]en '\e]4;7;?\a'                     # ^[]4;2;rgb:0000/6c6c/2020^G[pip@OniN4AM9WrH~/dvl/t8/raku]
      # ^[]4;7;rgb:d3d3/d7d7/cfcf^G[pip@OniN4AM9RSi~/dvl/t8/raku]         # [pip@OniN4AM9WsE~/dvl/t8/raku]en '\e]4;3;?\a'
      # [pip@OniN4AM9RV0~/dvl/t8/raku]en '\e]4;8;?\a'                     # ^[]4;3;rgb:f0f0/acac/4848^G[pip@OniN4AM9WvQ~/dvl/t8/raku]
      # ^[]4;8;rgb:5555/5757/5353^G[pip@OniN4AM9RYu~/dvl/t8/raku]
      # [pip@OniN4AM9Rcv~/dvl/t8/raku]en '\e]4;9;?\a'
      # ^[]4;9;rgb:efef/2929/2929^G[pip@OniN4AM9RgU~/dvl/t8/raku]
      # [pip@OniN4AM9Ri0~/dvl/t8/raku]en '\e]4;10;?\a'
      # ^[]4;10;rgb:8a8a/e2e2/3434^G[pip@OniN4AM9Rll~/dvl/t8/raku]
      # [pip@OniN4AM9RnS~/dvl/t8/raku]en '\e]4;11;?\a'
      # ^[]4;11;rgb:fcfc/e9e9/4f4f^G[pip@OniN4AM9Rqm~/dvl/t8/raku]
      # [pip@OniN4AM9Rrc~/dvl/t8/raku]en '\e]4;12;?\a'
      # ^[]4;12;rgb:7272/9f9f/cfcf^G[pip@OniN4AM9Ruf~/dvl/t8/raku]
      # [pip@OniN4AM9Rva~/dvl/t8/raku]en '\e]4;13;?\a'
      # ^[]4;13;rgb:f6f6/5858/e4e4^G[pip@OniN4AM9S12~/dvl/t8/raku]
      # [pip@OniN4AM9S2T~/dvl/t8/raku]en '\e]4;14;?\a'
      # ^[]4;14;rgb:3434/e2e2/e2e2^G[pip@OniN4AM9S5C~/dvl/t8/raku]
      # [pip@OniN4AM9S6R~/dvl/t8/raku]en '\e]4;15;?\a'
      # ^[]4;15;rgb:eeee/eeee/ecec^G[pip@OniN4AM9S9M~/dvl/t8/raku]
      # [pip@OniN4AM9SAa~/dvl/t8/raku]en '\e]4;16;?\a'
      # ^[]4;16;rgb:0000/0000/0000^G[pip@OniN4AM9SEG~/dvl/t8/raku]
      # [pip@OniN4AM9SJ9~/dvl/t8/raku]en '\e]4;17;?\a'
      # ^[]4;17;rgb:0000/0000/5f5f^G[pip@OniN4AM9SMd~/dvl/t8/raku]
      # [pip@OniN4AM9SP2~/dvl/t8/raku]en '\e]4;18;?\a'
      # ^[]4;18;rgb:0000/0000/8787^G[pip@OniN4AM9SSc~/dvl/t8/raku]
      # [pip@OniN4AM9Sfx~/dvl/t8/raku]en '\e]4;19;?\a'
      # ^[]4;19;rgb:0000/0000/afaf^G[pip@OniN4AM9SjI~/dvl/t8/raku]
      # [pip@OniN4AM9SkK~/dvl/t8/raku]en '\e]4;20;?\a'
      # ^[]4;20;rgb:0000/0000/d7d7^G[pip@OniN4AM9Sok~/dvl/t8/raku]
      # [pip@OniN4AM9Spj~/dvl/t8/raku]en '\e]4;21;?\a'
      # ^[]4;21;rgb:0000/0000/ffff^G[pip@OniN4AM9StE~/dvl/t8/raku]
      # [pip@OniN4AM9SvF~/dvl/t8/raku]en '\e]4;22;?\a'
      # ^[]4;22;rgb:0000/5f5f/0000^G[pip@OniN4AM9T01~/dvl/t8/raku]
      # [pip@OniN4AM9T27~/dvl/t8/raku]en '\e]4;23;?\a'
      # ^[]4;23;rgb:0000/5f5f/5f5f^G[pip@OniN4AM9T55~/dvl/t8/raku]
      # [pip@OniN4AM9T66~/dvl/t8/raku]en '\e]4;24;?\a'
      # ^[]4;24;rgb:0000/5f5f/8787^G[pip@OniN4AM9TDE~/dvl/t8/raku]
      #       HTTPS://StackOverFlow.Com/questions/15682537/ansi-color-specific-rgb-sequence-bash has response with the following HSV to RGB utilz:
      # After playing with nice tools found on xterm's source tree, here is how vttests/256colors2.pl show on my gnome-terminal: vttests/256colors2.pl
      #   shows 256 colors: 16 terminal colors + 6 * 6 * 6 RGB levels + 24 grayscales.
  ...
  unless(exists($self->{'_sprm'}{$spid})){ # could eventually also add scales, pal8s, etc.
    ...
    my @spis=split(//,      $spid    ); #"$ltxd[$cndx]$lfcd[$cndx]$lbcd[$cndx]$lfvd[$cndx]" # MaKe a SPrite for the joined col8 layers
      ...
      #@brgb=split(//,$pmap{$spis[2]}) if($#spis >= 2 && defined($spis[2]) && exists($pmap{$spis[2]}));for(@brgb){$_=b10($_)*4;}
       @frgb=(255,255,255); # try to load at least first 8pal8 plane from a8 map instead of old b64 RGB pmap set, but will still need new layer for DALOH
       @brgb=(  0,  0,  0); #   or can load b256 into Fclr && bclr? could mAB l8r support b256 f0ntz too, aftr more are made;
    if($#spis >= 1 && defined($spis[1])){my $flyr='F';if($spis[1]!~ /^[0-9A-Za-z._]$/){my $indx=b210($spis[1]);my $lndx=int(($indx % 256)/64);
                                            $flyr=substr('FDAL',$lndx,1);$spis[1]=b64($indx % 64);} if(exists($p8k2{$flyr}{$spis[1]})){
      $frgb[0]=hex(substr($x256[$p8k2{$flyr}{$spis[1]}],0,2));
      $frgb[1]=hex(substr($x256[$p8k2{$flyr}{$spis[1]}],2,2));
      $frgb[2]=hex(substr($x256[$p8k2{$flyr}{$spis[1]}],4,2)); }}
    if($#spis >= 2 && defined($spis[2])){my $blyr='F';if($spis[2]!~ /^[0-9A-Za-z._]$/){my $indx=b210($spis[2]);my $lndx=int(($indx % 256)/64);
                                            $blyr=substr('FDAL',$lndx,2);$spis[2]=b64($indx % 64);} if(exists($p8k2{$blyr}{$spis[2]})){
      $brgb[0]=hex(substr($x256[$p8k2{$blyr}{$spis[2]}],0,2));
      $brgb[1]=hex(substr($x256[$p8k2{$blyr}{$spis[2]}],2,2));
      $brgb[2]=hex(substr($x256[$p8k2{$blyr}{$spis[2]}],4,2)); }}
    $spis[3]='t'                   unless($#spis >= 3 && defined($spis[3]) && exists($GLBL{'_f8om'}{$spis[3]}));
    $self->{'_sprm'}{$spid}=  SDLx::Sprite->new(width => 8,height => 16); @brgb=(0,0,31);
    $self->{'_sprm'}{$spid}->surface->draw_rect([ 0, 0,  8,          16],[@brgb,255]);
    my $fhit=   @{$GLBL{'_f8om'}{$spis[3]}->{'f0ntgd8a'}[ord($spis[0])]}; # array of rows should be f0nt hite (&&should also be same in 'f0nthedr')
    for   my $rndx (0..$fhit){ # Row   iNDeX
      for my $pndx (0..    7){ # Pixel iNDeX
        if(     @{$GLBL{'_f8om'}{$spis[3]}->{'f0ntgd8a'}[ord($spis[0])]} > $rndx &&
          substr ($GLBL{'_f8om'}{$spis[3]}->{'f0ntgd8a'}[ord($spis[0])][$rndx],$pndx,1) eq '#'){my $prwd=1;my $prht=1;
...
# 2du:add compress option to reverse behavior,add maximize option to scale up to widest width multiple within $HWid or hIt up2 $HHit,mA nEd2DcOdUTF8&&out8;
  ...
  $cc=Oct::c8->new(  'cmnd' =>  1);$sz=2;$sz=shift(@_) if(@_ && $_[0]=~ /^\d+$/);$sb=$sm=$ss=0; # hopefully the regex will limit param to digit sizes
  ...
  for(0 ..  length($cc->{'_Stxt'})  -1){ # nEd2bypas yet prEsrv anyMbeded SKpz && just duplic8 Ech nrml tXt character x $sz thN split /\n/ 2 duplic8 $sz lInez
    if     (       $sm                         ){$ln .= substr($cc->{'_Stxt'},$_,1); # [pip@OniK3TM0bOH~]xx # Use of uninitialized value in subtraction (-) at
      if   (substr($cc->{'_Stxt'},$_,1) eq  'm'){              $sm  =   0;}}         #   /home/pip/lib/Oct/c8.pm line 1057. ## would be on 1058 now;
...
package Oct::c8::Fclr; # allow color arrays to be tied in parallel beyond just base tied text
...
sub STORE    {
  $_[0]->[0]->{'_Fclr'}->[$_[1]] = $_[2];
  $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'});}
sub STORESIZE{
  splice(@{$_[0]->[0]->{'_Fclr'}}, $_[1], @{$_[0]->[0]->{'_Fclr'}} - $_[1]);
  $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'});}
sub EXISTS   { return(0) unless(defined($_[0]->[0]->{'_Fclr'}->[$_[1]])); return(1); }
sub CLEAR    {             @{$_[0]->[0]->{'_Fclr'}} = ();         $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'}); }
sub PUSH     {        push(@{$_[0]->[0]->{'_Fclr'}}, $_[1]);      $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'}); }
sub POP      { $_ =    pop(@{$_[0]->[0]->{'_Fclr'}});             $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'}); return($_); }
sub SHIFT    { $_ =  shift(@{$_[0]->[0]->{'_Fclr'}});             $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'}); return($_); }
sub UNSHIFT  {     unshift(@{$_[0]->[0]->{'_Fclr'}}, $_[1]);      $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'}); }
sub SPLICE   {
#              $_ = splice(@{$_[0]->[0]->{'_Fclr'}}, @_[1..$#_]); $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'}); return($_); }
  my $self = shift;
  my $offs = shift || 0;
  my $leng = shift; $leng = $self->[0]->FETCHSIZE() - $offs  unless(defined($leng));
  my $retn = splice(@{$self->[0]->{'_Fclr'}}, $offs, $leng, @_);
  $self->[0]->Oct::c8::TestDraw() if($self->[0]->{'_flagadtb'});return($retn);}
sub EXTEND   { }
package Oct::c8::bclr;
...
sub STORE    {
  $_[0]->[0]->{'_bclr'}->[$_[1]] = $_[2];
  $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtb'});}
sub STORESIZE{
  splice(@{$_[0]->[0]->{'_bclr'}}, $_[1], @{$_[0]->[0]->{'_bclr'}} - $_[1]);
  $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtb'});}
sub EXISTS   { return(0) unless(defined($_[0]->[0]->{'_bclr'}->[$_[1]])); return(1); }
sub CLEAR    {             @{$_[0]->[0]->{'_bclr'}} = ();         $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtb'}); }
sub PUSH     {        push(@{$_[0]->[0]->{'_bclr'}}, $_[1]);      $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtb'}); }
sub POP      { $_ =    pop(@{$_[0]->[0]->{'_bclr'}});             $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtb'}); return($_); }
sub SHIFT    { $_ =  shift(@{$_[0]->[0]->{'_bclr'}});             $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtb'}); return($_); }
sub UNSHIFT  {     unshift(@{$_[0]->[0]->{'_bclr'}}, $_[1]);      $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtb'}); }
sub SPLICE   {
#              $_ = splice(@{$_[0]->[0]->{'_bclr'}}, @_[1..$#_]); $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtf'}); return($_); }
  my $self = shift;
  my $offs = shift || 0;
  my $leng = shift; $leng = $self->[0]->FETCHSIZE() - $offs  unless(defined($leng));
  ...
  $self->[0]->Oct::c8::TestDraw() if($self->[0]->{'_flagadtb'});return($retn);}
sub EXTEND   { }
package Oct::c8::f0nt; # allow f0nt arrays to be tied too
...
sub STORE    {
  $_[0]->[0]->{'_f0nt'}->[$_[1]] = $_[2];
  $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtn'});}
sub STORESIZE{
  splice(@{$_[0]->[0]->{'_f0nt'}}, $_[1], @{$_[0]->[0]->{'_f0nt'}} - $_[1]);
  $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtn'});}
sub EXISTS   { return(0) unless(defined($_[0]->[0]->{'_f0nt'}->[$_[1]])); return(1); }
sub CLEAR    {             @{$_[0]->[0]->{'_f0nt'}} = ();         $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtn'}); }
sub PUSH     {        push(@{$_[0]->[0]->{'_f0nt'}}, $_[1]);      $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtn'}); }
sub POP      { $_ =    pop(@{$_[0]->[0]->{'_f0nt'}});             $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtn'}); return($_); }
sub SHIFT    { $_ =  shift(@{$_[0]->[0]->{'_f0nt'}});             $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtn'}); return($_); }
sub UNSHIFT  {     unshift(@{$_[0]->[0]->{'_f0nt'}}, $_[1]);      $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtn'}); }
sub SPLICE   {
#              $_ = splice(@{$_[0]->[0]->{'_f0nt'}}, @_[1..$#_]); $_[0]->[0]->Oct::c8::TestDraw() if($_[0]->[0]->{'_flagadtn'}); return($_); }
  my $self = shift;
  my $offs = shift || 0;
  my $leng = shift; $leng = $self->[0]->FETCHSIZE() - $offs  unless(defined($leng));
  my $retn = splice(@{$self->[0]->{'_f0nt'}}, $offs, $leng, @_);
  $self->[0]->Oct::c8::TestDraw() if($self->[0]->{'_flagadtn'});return($retn);}
sub EXTEND   { }
...
Oct::c8 - col8 interface to classic or enhanced text-mode applic8ions

=head1 VERSION

This document8ion refers to version 0.0 of Oct::c8, which was released on d8VSD8vs.

=head1 SYNOPSIS

  #!/usr/bin/perl
  use        Oct::c8; # basic col8 applic8ion with
  $c=tie(@t,'Oct::c8'         ); # tied arrayz per layer
     tie(@F,'Oct::c8::Fclr',$c);
     tie(@b,'Oct::c8::bclr',$c); # ms  2 w8 4   Key BlO
     tie(@f,'Oct::c8::f0nt',$c);  sub  K{$c->GetK(2000)}
...
I hope you find Oct::c8 useful. Please feel free to e-mail me any suggestions or coding tips or notes of appreC8ion. Thank you. TTFN.
...
Revision history for Perl extension Oct::c8:
...
Assuming Oct gets released as a distribution on the CPAN, please run:

  `perl -MCPAN -e "install Oct"`
