# D8FMKTuW: Oct::d8.pm by PipStuart <Pip@CPAN.Org> to define simple objects for storing instants in time, adapted from the old poorly named Time::PT;
package     Oct::d8;
use warnings;use  strict;use utf8;use v5.22;use open qw[:std :encoding(UTF-8)];
# 2du:either duplic8 or just wrap `date` for rL8ive timez like: $(date -v Monday +"%Y%d%m") or `date -d 'this Monday'`, 'next Monday', 'Monday last week'…,
#   try to use or replic8 DateTime::Format::Strptime to help d8 support various strf && strp time formats,generally mimic l8st Time::Piece behavior,
...
#   stuD HTTPS://MetaCPAN.Org/module/App::Timer by Mohammad Anwar <Mohammad.Anwar@Yahoo.Com> duing tImingz on XeQtionz but I'd lIk 2du it wi d8 && dur8:
# END{my $te=time-$^T;my $mm=$te/60;my $ss=$te%60;my $hh=$mm/60;printf("ProgRan %02d:%02d:%02d (hms=TimeElapsed:$te since stRtIme:$^T);\n",$hh,$mm%60,$ss);};
# nOt:RFC3339 is probably mOstly a subset of ISO8601 but DfInz some Xtra fE8urz borOd from Lswher && is Usefuly visualIzd thru an upd8ng Venn dIagram Cn @:
#   HTTPS://IJMacD.GitHub.Io/rfc3339-iso8601 wich presNtly (NCALCUL0) shOz "2023-12-10 12:30:21-06:00" as Onlyin 3339 && 2023-12-10T18:30:21Z as UTC in bOth.
#   abov site makez it look lIk 3339 can 8601 =~ s/T/$st_/ (substitute Up 'T' wi ' ' spAce, low 't', undrscor '_', or mAB wutevr Ls && can have -00:00 Zone).
#   thOs R not alowd in iso8601, but othrwIse just any YYYY-MM-DD or hh:mm:ss(\.\d{1,3})?(Z|[-+]\d\d:\d\d) not includng -00:00 (&& joining thOs wi T) R bOth!
#   furthr 'ISO 8601-1:2019' DscrIbz a whol lot th@ is not in 3339 (like Week#,Periods Year|Month|Week|Day|Time,Ranges start_date(TtimeZ)?/(end_date|Period);
# O31LHZon:tsdata 2024a-0ubuntu0.22.04 348KB just rElEsd && shud B proly gud 2 try 2 upd8 l8St zOne DtAilz 2 sinc it haz Bn a long tIm sinc setng thMup orig;
require     Oct::d8::fldz; # probably l8r also want to use AUTOLOAD to wrap class object member method functions like expand() && xpnd() around: full();
#equire                        Exporter;
use base qw(Oct::d8::fldz);#porter);
use         Oct::d8::dur8;
use         Oct::a8;
use         Oct::b8;
use         Carp;
...
sub _cmp_num{my($larg,$rarg,$srvr)=@_;
  ($larg,$rarg)=($rarg,Oct::d8->new($larg)) if($srvr); # mk both args d8 objects
  $rarg=Oct::d8->new($rarg) unless(ref($rarg) && $rarg->isa('Oct::d8'));
  if   (($larg->Y < $rarg->Y) ||
        ($larg->M < $rarg->M) ||
        ($larg->D < $rarg->D) ||
...
sub _add{
  my($larg,$rarg,$srvr)=@_;my $rslt=Oct::d8->new('');
    ($larg,$rarg)=($rarg,Oct::d8::dur8->new($larg)) if($srvr);
           $rarg =       Oct::d8::dur8->new($rarg)  unless(ref($rarg) && $rarg->isa('Oct::d8::dur8'));
...
# d8 - anything else is not supported yet
sub _sub{
  my($larg,$rarg,$srvr)=@_;my $rslt;          $larg=Oct::d8->new($larg) if($srvr);
  if(ref($rarg)&& $rarg->isa('Oct::d8')){$rslt=Oct::d8::dur8->new(     );
  }else                                      {$rarg=Oct::d8::dur8->new($rarg) unless(ref($rarg)&& $rarg->isa('Oct::d8::dur8'));
                                              $rslt=Oct::d8->new(''   );}
  $rslt->{'p'}=$larg->p-$rarg->p;
  $rslt->{'s'}=$larg->s-$rarg->s;
  $rslt->{'m'}=$larg->m-$rarg->m;
  ...
  $rslt->{'M'}=$larg->M;
  $rslt->{'Y'}=$larg->Y;
  $rslt->_sift( ) if($rslt->isa('Oct::d8'));
  $rslt->{'M'}=$larg->M-$rarg->M;
  $rslt->{'Y'}=$larg->Y-$rarg->Y;
  $rslt->_sift(1) if($rslt->isa('Oct::d8'));
  return($rslt);}
...
# Note:`d8` && `d8ok` utilz use 'f' for Full Format to invoke this original expand form (e was phased out since Oct almost always uses x for eXpand now);
sub epoc{ # returns a d8 object's epoch float       seconds value
  my $self=shift(@_);my $xpop=shift(@_)||'';
  if(defined($self)&&      $self->e ){ # trying to just call ->e was still geting undef subroutine &Oct::a8::e calld@/home/pip/lib/Oct*/d8.pm ln:182;
    return(                $self->e );}}
    ...
    while ( $srcf   ) {
      if(-f $srcf && -r $srcf) { $dstf = $srcf; $extn = '';
        ...
        if(@tsts) { @tsts = @tsts[0..7] if(@tsts > 8); # test last 8 backups for differences before proceeding (originally was just 3)
          for(@tsts) {                                                  #          below also needed fix to wrap with dbl-quotez above in glob() too!;
            if($osty eq 'good') { $pass =  `diff --brief "$_" "$srcf"`; # NA5M3bak:minor upd8 2 dbl-quote fIlnAmez 4 diff callz (sinc faild on spAcz);
...
sub new{my($nvkr,$ityp,$idat)=@_;my $nobj=ref($nvkr);
  ...
  my $self=Oct::d8::fldz->new($clas);my $rgxs;my $mont;my $zfou=0;my($lofs,$hofs,$mofs);my @attz=$self->_attribute_names();
...
Oct::d8 - objects to store a dated phass (60th-of-a-second) moment in time

=head1 VERSION

This document8ion refers to version 0.0 of Oct::d8, which was released on d8VSD8vs.

=head1 SYNOPSIS

  use       Oct::d8;use v5.10;
  my  $d8 = Oct::d8->new();
  say 'd8 is: ',     $d8       ;
...
By default, Oct::d8 stores time descriptions precise to
...
Hietaniemi <jhi@iki.fi>. Oct::d8 inherits base
data structure && object methods from L<Oct::d8::fldz>.
d8 was written to simplify storage && calcul8ion
of encoded, yet distinct && human-readable, time d8a
objects.

This module (Oct::d8) does not replace the standard localtime &&
gmtime functions like L<Time::Piece> but Oct::d8 objects behave
...
distinct fields && configurable precision. Thus, L<Oct::d8::fldz>
was written to be the abstract base class for:

  Oct::d8::dur8  ( crE8s objects which represent dur8ions of time )
      &&
  Oct::d8        ( crE8s objects which represent instants in time )
...
Many of Oct::d8's methods have been patterned after the excellent
L<Time::Piece> module written by Matt Sergeant <matt@sergeant.org>
&& Jarkko Hietaniemi <jhi@iki.fi>.

=head2 new(<InitType>, <InitData>)

Oct::d8's constructor can be called
...
initialize d8 objects several different ways:
  ...
  * <packedB64InitStringImplies'd8'>
    e.g., Oct::d8->new('E7UM23JF');
  * 'd8'     => <packedB64InitString>
    e.g., Oct::d8->new('d8'    => '0A1B2C3D');
  * 'list'   => <arrayRef>
    e.g., Oct::d8->new('array' => [0, 1, 2..7]);
  * 'hash'   => <hashRef>
    e.g., Oct::d8->new('hash'  => {'s' => 8, 'Y' => 2003})
  * 'expand' => <expandStr>
    e.g., Oct::d8->new('expand'=>'Wed Jul 30 02:33:58:45 -0500 2014')
  * 'lsft'   => <lsFullTimeStr>
    e.g., Oct::d8->new('lsft'  =>'2013-08-15 12:55:23.833703078 -0500')
  * 'cpan'   => <cpanStr>
    e.g., Oct::d8->new('cpan'  =>'Wed, 14 Aug 2013 15:08:07 GMT')
  * 'iso'    => <isoStr>
    e.g., Oct::d8->new('iso'   =>'1997-07-16T19:20:30.45+01:00')
...
Returns the Oct::c8 color code string approprE8 for d8 d8a.
...
simple localiz8ion.
  ...
  my $hebrew_day = Oct::d8->wdayname(@days);
                 # Oct::d8->monname() can be used similarly

To update the global lists, use:

  Oct::d8::day_list(@days);
    &&
  Oct::d8::mon_list(@months);
...
It's possible to use simple addition and subtraction of objects:
  ...
  my $cur_d8       = Oct::d8->new(); # YMDzhmsp
  my $one_week     = Oct::d8::dur8->new('700000');
...
The following are valid (where $t0 and $t1 are Oct::d8 objects
&& $dr is an Oct::d8::dur8 object):

  $t0 - $t1;  # returns Oct::d8::dur8 object
  $t0 - '63'; # returns Oct::d8       object
  $t0 + $dr;  # returns Oct::d8       object
...
Whenever individual Oct::d8 attributes are going to be
...
I hope you find Oct::d8 useful.

=head1 CHANGES

Revision history for Perl extension Oct::d8:
...
* added exported   d8color() to be reverse of Oct::b8::b8color()
...
* updated POD links && CHANGES chrono-logy
...
L<Oct::b8>          to handle simple number-base conversion

L<Time::DayOfWeek>       also stores global day && month names

L<Time::DaysInMonth>

L<Oct::d8::fldz>    to provide underlying object structure

L<Oct::d8::dur8>    to represent spans of time

Oct::d8 uses (if available):
