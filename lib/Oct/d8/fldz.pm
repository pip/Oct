# D8FMMHPM:Oct::d8::fldz.pm crE8d by PipStuart <Pip@CPAN.Org> as a base class for specialized d8 objects (Oct::d8 and Oct::d8::dur8);
package    Oct::d8::fldz;
use warnings;use strict;use utf8;use v5.22;use open qw[:std :encoding(UTF-8)];
use vars qw( $AUTOLOAD );
my $VERSION='0.0';my $d8VS='NBSL8UTF';
use overload
...
sub w8{my $self=shift(@_);my $dur8;my $durs; # crE8 a new 'wait' method which should maybe just export from dur8.pm instead? expects dur8 obj or dur8 str param
  if  ( $self->isa('Oct::d8::dur8')){$dur8=$self;} # from HTTPS://PerlDoc.Perl.Org/functions/ref.html && ../perlobj.html betr2 UNIVERSAL->isa($class)
  else{   $dur8=shift(@_) || '0';
    if(!$dur8->isa('Oct::d8::dur8')){$dur8=~ s/[^0-9A-Za-z._]+//g; # strip non-b64 characters before trying to crE8 a new dur8 object from string param
          $dur8=  Oct::d8::dur8->new($dur8);}} # 2du:allow float b64 phasses ('F%W') above or in dur8 new to enable more precise HiRes w8ing here
...
# Oct::d8::fldz object constructor as class method or copy as object method.
      ...
      if    ($ityp=~ /^s/i){ # 'str'
        ...
        for(keys(%{$idat})){
          if($attr=~ /$_/){
            $self->{$attr}=$idat->{$_};
            delete($idat->{$_});}}
      }else{croak "!*EROR*! Oct::d8::fldz::new initialization type: $ityp did not match 'str', 'array', or 'hash'!\n";}} # undetected init type
  }return($self);}
...
Oct::d8::fldz - abstract objects to store distinct d8 and time fields

=head1 VERSION

This documentation refers to version pAKGvERS of Oct::d8::fldz, which was released on d8VSD8vs.

=head1 SYNOPSIS

  package Oct::d8::fldz::NewChildPackageOfd8fldz;
  use base qw(Oct::d8::fldz);

  # NewChildPackageOfd8fldz definition...

=head1 DESCRIPTION

Oct::d8::fldz defines simple d8time objects with 8 distinct fields for:
...
fldz d8a and methods are meant to be inherited by other classes (namely L<Oct::d8> and L<Oct::d8::dur8>) which implement specific useful
interpret8ions of individual Oct::d8::fldz.
...
Oct::d8::fldz was written to be the abstract base class for:

  Oct::d8        ( crE8s objects for instants in time )
    and
  Oct::d8::dur8  ( crE8s objects for dur8ions of time )

=head1 USAGE

Many of Oct::d8::fldz's methods have been patterned after the excellent L<Time::Piece> module written by Matt Sergeant <matt@sergeant.org> and
Jarkko Hietaniemi <jhi@iki.fi>.

=head2 new(<InitType>, <InitData>)

Oct::d8::fldz's constructor can be called as a class method to create a brand new object or as an object method to copy an existing object.
Beyond that, new() can initialize fldz objects the following ways:

  * <packedB64InitStringImplies'str'>
    eg. Oct::d8::fldz->new('01234567');
  * 'str'  => <packedB64InitString>
    eg. Oct::d8::fldz->new('str'  => 'ABCDEFGH');
  * 'array'=> <arrayRef>
    eg. Oct::d8::fldz->new('array'=> [0, 1, 2..7]);
  * 'hash' => <hashRef>
    eg. Oct::d8::fldz->new('hash' => {'phass' => 7, 'year' => 2014})
...
This implied 'str'-type initializ8ion will probably be the most common Oct::d8::fldz object crE8ion mechanism when individual fields do not exceed 64
since this efficient represent8ion is why the module was crE8d.

The following methods allow access to individual fields of existent Oct::d8::fldz objects:
...
Whenever individual Oct::d8::fldz attributes are going to be printed or an entire object can be printed with multi-colors,
...
Even though Oct::d8::fldz is designed to be an abstract base class, it has not been written to croak on direct usage and object
instantE8ion because simple fldz objects may already be worthwhile.

I hope you find Oct::d8::fldz useful. Please feel free to e-mail me any suggestions or coding tips or notes of appreC8ion. Thank you. TTFN.

=head1 CHANGES

Revision history for Perl extension Oct::d8::fldz:
...
* migr8d old Time::Fields over to new Oct::d8::fldz name
...
Oct::d8::fldz requires:

L<Carp>                to allow errors to croak() from calling sub

L<Oct::b8>        to handle number-base transl8ion

Oct::d8::fldz utilizes (if available):
