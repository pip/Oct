# D8FMMtLL:  Oct::d8::dur8.pm by PipStuart <Pip@CPAN.Org> to make simple objects for Oct's dur8ions (spans or overloaded "frames") of time.
package      Oct::d8::dur8;
...
sub _cmp_num{
  my($larg,$rarg,$srvr)=@_;
  ($larg,$rarg)=($rarg,Oct::d8::dur8->new($larg)) if($srvr); # mk both args dur8 objects
...
sub _add{
  ...
  $larg=Oct::d8->new($larg) if($srvr);
  $rarg=Oct::d8->new($rarg) unless(ref($rarg)&& $rarg->isa('Oct::d8::dur8'));
  if((ref($larg)&& $larg->isa('Oct::d8'))||
     (ref($rarg)&& $rarg->isa('Oct::d8'))){
    $rslt=$larg+$rarg; # pass off calculation to d8.pm
  }else{
    $rslt=Oct::d8::dur8->new(); # 2du:maybe also attempt to handle math on orig 't' HiRes::time() epoch float secs && 'e' specified epoch fsecs too?;
...
# Oct::d8::dur8 object constructor as class method or copy as object method.
        ...
        if    ($ityp=~ /^s/i){ # 'str'
          ...
          for(keys(%{$idat})){ # do some searching to find hash key that matches
            if($attr=~ /$_/){$self->{$attr}=$idat->{$_};
              delete(                       $idat->{$_});}}
        }else{croak "!*EROR*! Oct::d8::dur8::new initialization type: $ityp did not match 'str','array','hash','g3', or 'mp'!\n";}}} # unDtectd init type
  }return($self);}
sub total_phasses{ # return the integer number of phasses in a Oct::d8::dur8 obj
...
Oct::d8::dur8 - objects to store a dur8ion of time

=head1 VERSION

This documentation refers to version pAKGvERS of Oct::d8::dur8, which was released on d8VSD8vs.

=head1 SYNOPSIS

  use Oct::d8::dur8;

  my $d = Oct::d8::dur8->new('verbose' => '2 weeks');
...
Oct::d8::dur8 inherits base data structure and object methods from Oct::d8::fldz.
...
distinct fields and configurable precision. Thus, L<Oct::d8::fldz> was written to be the abstract base class for:
...
Many of Oct::d8::dur8's methods have been patterned after the excellent L<Time::Piece> module written by Matt Sergeant <matt@sergeant.org>
and Jarkko Hietaniemi <jhi@iki.fi>.

=head2 new(<InitType>, <InitData>)

Oct::d8::dur8's constructor can be called as a class method to create a brand new object or as an object method to copy an existing object.
Beyond that, new() can initialize dur8 objects in the following ways:

  * <packedB64InitStringImplies'str'>
    eg. Oct::d8::dur8->new('01234567');
  * 'str'  => <packedB64InitString>
    eg. Oct::d8::dur8->new('str'  => 'ABCDEFGH');
  * 'array'=> <arrayRef>
    eg. Oct::d8::dur8->new('array'=> [0, 1, 2..7]);
  * 'hash' => <hashRef>
    eg. Oct::d8::dur8->new('hash' => {'phass' => 7, 'year' => 2014})
  * 'g3'   => <gst123TimeString>
    eg. Oct::d8::dur8->new('g3'   => '0:03:47.88')
  * 'mp'   => <mplayerFloatSecondz>
    eg. Oct::d8::dur8->new('mp'   =>  254.1)

=head2 total_phasses()

total_phasses simply returns the total number of phasses an Oct::d8::dur8 object specifies.
...
The following methods allow access to individual fields of Oct::d8::dur8 objects:
...
Please see L<Oct::d8::fldz> for a more thorough description of field accessor methods.

=head1 NOTES

Whenever individual Oct::d8::dur8 attributes are going to be printed or an entire object can be printed with multi-colors,
...
I hope you find Oct::d8::dur8 useful. Please feel free to e-mail me any suggestions or coding tips or notes of appreC8ion. Thank you. TTFN.
...
* migr8d old Time::Frame over to new Oct::d8::dur8 name and lowercased && shortened fldz.pm
